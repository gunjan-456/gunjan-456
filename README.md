<h1 align="left">Hi👋, I'm Gunjan Singh</h1>
<h4 align="left">
🚀 MERN Stack Developer | Backend Enthusiast | Problem Solver
</h4>

---

<h2>✨About me</h2>

<p>I’m a passionate full stack developer eager to apply my skills in real-world projects. I enjoy learning new technologies, writing clean code, and building user-friendly applications. I’m excited to grow, collaborate, and contribute to meaningful software.</p>

---




## 🛠 Tech Stack & Skills

| 🧠 Languages | 🌐 Web Technologies | 🧰 Tools | 🔐 Auth & Testing |
|-------------|------------------|---------|------------------|
| JavaScript, Java | React, Redux, Node.js, Express, MongoDB, MySQL | Git, VS Code | REST APIs, Postman |

---

## Experience


### 💼 Web Developer Intern  
TechnoHacks, Remote (Nov 2023 – Jan 2024)

- Enhanced skills in HTML, CSS, and JavaScript through practical projects
- Completed assigned tasks efficiently, demonstrating adaptability and time management
- Implemented core features, database operations, and UI components.


---

## 📬 Let's Connect and Create Impactful Software Together!

 📩Email: gunjansingh72002@gmail.com
 <br>
 🖇️linkedin.com/in/gunjan-singh-3620b822a
  

---





###
